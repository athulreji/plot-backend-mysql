...
module.exports.buy=async(req,res)=>{
    ...
    try {
        ...
        connection.query(query,function(error,data) {
            ...
            query = `select * from user_shares where user_id=${user_id} and share_id=${share_id}`;
            connection.query(query, function(error, data) {
                if(data.length==0) {
                    query = `insert into user_shares(user_id,share_id, share_count) values(${user_id}, ${share_id},${number})`;
                }
                else {
